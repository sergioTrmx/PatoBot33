...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  ...
  .then(batch => {
    ...
    conn.reply(m.chat, `*Datos Covid-19 Provinsia ${batch.features[0].attributes.Provinsi}*\n\nCasos positivos = ${format(batch.features[0].attributes.Kasus_Posi)}\nCasos curados = ${format(batch.features[0].attributes.Kasus_Semb)}\nCasos de muerte = ${format(batch.features[0].attributes.Kasus_Meni)}`, m)
