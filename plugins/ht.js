let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    conn.sendFile(m.chat, img, 'Harta Tahta.png', '*PatoV3*\nMade with FFmpeg', m)
  } finally {
...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    let layers = [
      ...
      textArgs('Pato', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2-(text_h*${lh})`, w, h) + format + '[top]',
      textArgs('V3', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2`, w, h) + format + '[mid]',
