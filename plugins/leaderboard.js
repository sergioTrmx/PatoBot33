let handler = async (m, { conn, args }) => {
  ...
  let text = `
• *Top de xp ${len}* •
Usted: *${usersExp.indexOf(m.sender) + 1}* Usuarios *${usersExp.length}*

${sortedExp.slice(0, len).map(([user, data], i) => (i + 1) + '. @' + user.split`@`[0] + ': *' + data.exp + ' Exp*').join`\n`}

• *Top de limit ${len}* •
Usted: *${usersLim.indexOf(m.sender) + 1}* usuarios *${usersLim.length}*
...
handler.help = ['leaderboard [usuarios]', 'lb [usuarios]']
handler.tags = ['xp']
